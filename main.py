...
def register():
    ...
    if register_form.validate_on_submit():
        user = db.session.execute(db.select(User).where(User.email == register_form.email.data)).scalars().first()
        if user:
            flash("The email already exists, please login instead.")
...
def show_post(post_id):
    ...
    return render_template("post.html", post=requested_post, logged_in=current_user.is_authenticated,is_admin=is_admin, form=comment_form, comments=all_comments)
